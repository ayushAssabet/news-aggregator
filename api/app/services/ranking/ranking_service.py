...
def _category_boost(cat: Optional[models.ArticleCategory], boosts: Dict[str, float]) -> float:
...
def _entity_boost(article: models.Article) -> float:
    """
...
def _topic_boost(db: Session, article: models.Article, hours: int = 24) -> float:
    """
    ...
    if not hasattr(models.Article, "keywords"):
        return 0.0
    ...
    stmt = select(models.Article.keywords).where(models.Article.published_at >= cutoff)
...
def compute_score(article: models.Article, db: Session, cfg: RankingConfig) -> Dict[str, Any]:
...
def rank_articles(
    ...
    *,
    ...
    conditions = [models.Article.published_at >= cutoff]
    if require_summary:
        conditions += [models.Article.summary != None, models.Article.summary != ""]
